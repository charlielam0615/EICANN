...
class EINet(bp.dyn.Network):
    def __init__(self):
        ...
        # synapses
        self.E2I = bp.dyn.ExpCUBA(pre=E, post=I, conn=bp.conn.FixedProb(prob), tau=tau_Es, g_max=JEI/bm.sqrt(num))
        self.E2E = bp.dyn.ExpCUBA(pre=E, post=E, conn=bp.conn.FixedProb(prob), tau=tau_Es, g_max=JEE/bm.sqrt(num))
        self.I2I = bp.dyn.ExpCUBA(pre=I, post=I, conn=bp.conn.FixedProb(prob), tau=tau_Is, g_max=JII/bm.sqrt(num))
        self.I2E = bp.dyn.ExpCUBA(pre=I, post=E, conn=bp.conn.FixedProb(prob), tau=tau_Is, g_max=JIE/bm.sqrt(num))
...
bp.visualize.line_plot(runner.mon.ts, 5e1*bm.mean(runner.mon['E.spike'].astype(bm.float32),axis=1), xlim=(0, duration))  
