...
from unbalanced_network import UnbalanceNet
...
jee = 2.5 * ei_scale
jei = 5.0 * ei_scale
JIE = jie / (size_Id*prob)
JII = jii / (size_Id*prob)
JEE = jee / (size_E*prob)
JEI = jei / (size_E*prob)
gl = 0.
...
def run(exp_id):
    net = UnbalanceNet(num_exc=size_E, num_inh=size_Id, tau_E=tau_E, tau_I=tau_I, tau_Ef=tau_Ef, tau_If=tau_If,
    ...
    # init input
    Einp_scale = num_ff * f_E / num_ff
    Iinp_scale = num_ff * f_I / num_ff
