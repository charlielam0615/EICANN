...
def noisy_input_protocol(amplitude, duration, dt=global_dt):
    ...
    ramp_noise = bm.sqrt(ramp_amp[:, None]*E_bump[None,]+bg_str*bm.ones([1, size_E])) * bm.sqrt(dt) * bm.random.randn(ramp_T, size_E)
    hold_noise = bm.sqrt(hold_amp*E_bump[None,]+bg_str*bm.ones([1, size_E])) * bm.sqrt(dt) * bm.random.randn(hold_T, size_E)
    ...
    ramp_noise = bm.sqrt(ramp_amp[:, None]*I_bump[None,]+ bg_str*bm.ones([1, size_I])) * bm.sqrt(dt) * bm.random.randn(ramp_T, size_I)
    hold_noise = bm.sqrt(hold_amp*I_bump[None,]+bg_str*bm.ones([1, size_I])) * bm.sqrt(dt) * bm.random.randn(hold_T, size_I)
