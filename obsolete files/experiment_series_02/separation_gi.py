...
class EICANN(bp.dyn.Network):
    def __init__(self):
        self.conn_a = bm.sqrt(2 * bm.pi) * (bm.pi*2)
        self.stim_a = bm.sqrt(2 * bm.pi) * (bm.pi / 3)
        self.J = 1.
        self.A = 1.
...
# ===== Persistent Activity ====
bg_amp = bg_str * net.A
st_amp = bp.inputs.section_input(values=[[bg_amp]], durations=[1000.], dt=global_dt)
ramp_amp = bp.inputs.ramp_input(c_start=0, c_end=1-bg_str, duration=1., dt=global_dt)
hold_amp = bp.inputs.section_input(values=[[1-bg_str]], durations=[1449.], dt=global_dt)
remove_amp = bp.inputs.section_input(values=[[bg_amp]], durations=[900.], dt=global_dt)
...
# plt.plot([1000, 3550], [int(size_E/2), int(size_E/2)], label='input peak', color='red')
plt.plot([1000, 3550], [int(size_E/4), int(size_E/4)], label='input peak', color='red')
plt.plot([1000, 3550], [int(size_E*3/4), int(size_E*3/4)], label='input peak', color='red')
