...
cann_scale = 0.0125 * 6
gEE_G = 0.065 * 300 * 8 * cann_scale
gEIp = 0.0175 * 750 * 6 * cann_scale
gIpE = -0.1603 * 250 * 4 * cann_scale
...
eicann_inputs = eicann.get_stimulus_by_pos(0.)
eicann_bg_inputs = bg_str * eicann.A
eicann_inputs, eicann_dur = bp.inputs.section_input(values=[eicann_bg_inputs, eicann_inputs+eicann_bg_inputs, eicann_bg_inputs],
                                             durations=[20., 50., 50.],
                                             return_length=True,
                                             dt=global_dt)
eicann_mean_inputs = bm.mean(eicann_inputs, axis=-1, keepdims=True)
noise = bm.random.randn(*eicann_inputs.shape) * 0 # * inputs
eicann_E_inp = Einp_scale * (eicann_inputs+noise)
eicann_I_inp = Iinp_scale * eicann_mean_inputs


# ===== SCANN ====

scann_inputs = scann.get_stimulus_by_pos(0.)
scann_bg_inputs = 0.
scann_inputs, scann_dur = bp.inputs.section_input(values=[scann_bg_inputs, scann_bg_inputs+scann_inputs, scann_bg_inputs],
                                         durations=[20., 50., 50.],
                                         return_length=True,
                                         dt=global_dt)
scann_I_inputs = bm.mean(scann_inputs, axis=-1, keepdims=True)
noise = bm.random.randn(*scann_inputs.shape) * 0
scann_E_inp = Einp_scale * (scann_inputs+noise)
scann_I_inp = Iinp_scale * scann_I_inputs
...
eicann_runner = bp.dyn.DSRunner(eicann,
                                 ...
                                 inputs=[('E.ext_input',  eicann_E_inp, 'iter', '='),
                                         ('Id.ext_input', eicann_I_inp*0.5, 'iter', '='),
                                         ('Ip.ext_input', eicann_I_inp*0.5, 'iter', '=')],
                                 dt=global_dt)
...
nm_eicann_readout = eicann_readout.T / bm.mean(eicann_readout[:,2000:7000])
...
nm_scann_readout = scann_readout.T / bm.mean(scann_readout[:,2000:7000])
